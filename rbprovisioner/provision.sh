...
proxy_start () {
  ...
  OUT=$(docker run -d --name="nginx-proxy" -v "/var/run/docker.sock:/tmp/docker.sock:ro" -v "/data/nginx-proxy/conf.d/nginx_custom.conf:/etc/nginx/conf.d/nginx_custom.conf:ro" -p "80:80" -p "443:443" jwilder/nginx-proxy:0.3.0)
  echo "------------------------------------"
  echo
}
